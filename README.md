...
天气来源:在日本用日本气象厅(JMA)——逐时和每日用 JMA 自己的 MSM/GSM 模式(经 Open-Meteo 的 `models=jma_seamless`),降水概率和每日天气用 JMA 官方预报(6 小时降水概率、週間天気予報),「现在」用最近的 AMeDAS 观测站(每 10 分钟,测到雨就算下雨)。JMA 任何一部分拿不到都自动退回原来的 Open-Meteo 默认预报;日本以外照旧用 Open-Meteo。
...
- Shift 单按切中 / 英，切换时在光标旁提示「英文 / 中文」；中 / 英对所有程序是同一个开关（「按程序默认英文」的程序各自记）。
- Caps Lock：默认锁定大写，打开时字母、数字、标点按键盘原样交给程序，提示「大写」。日文（JIS）键盘上单按「英数 / Caps Lock」键也一样（日文键盘布局下单按它只是「英数」、要 Shift 才是 Caps Lock，输入法替你切换）；Shift+Caps Lock 不会顺带切到英文。设置页可改成像 Mac 一样切到英文（字母小写、Shift 大写，关掉回到中文）。
- 焦点不在输入框（Chrome 里点在网页空白处、密码框等程序不收文字的地方）时按键原样交给程序（网页快捷键照常），不开始组字。
- **颜文字**（设置页可关）：像 Emoji 一样出现在候选里（标「颜」）——打「开心」「晚安」「ありがとう」给出相关的，打一整句「中秋节快乐」按句子里的词找（先赏月的，再开心的）；一个词时替换这个词，一整句时接在句子后面。数据是近几年日本常用的颜文字加上 Mozc（Google 日语输入法）的颜文字表，配了中文和日文关键词：`tools/ime-kaomoji/popular.tsv` 可以自己加，再跑 `mkkaomoji.py` 生成 `ime/engine/data/kaomoji.tsv`（`ime/engine/src/kaomoji.rs`）。
- **联想输入**（设置页可关）：中文词上屏后候选窗接着给出下一个可能打的词，数字键选、别的键收起（这时打数字用小键盘）。先按你自己打过的「这个词后面接什么」（边打边学，`nextword.log`；用 `tools/ime-bench/mkpersonal.py` 从自己的文档生成的 `nextword_personal.tsv` 也会读），再按雾凇词库里以这个词开头的常用词组（句子末尾最长的那个词作上下文，「今天天气」→ 天气 → 预报 / 很好）。词库在引擎启动后后台加载（约半秒）（`ime/engine/src/predict.rs`）。
